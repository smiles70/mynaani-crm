...
function explain(error: string): string {
	switch (error) {
		case "no_user_grant":
			return "This workspace did not grant Mynaani permission to add itself to a private channel.";
		case "channel_not_found":
			return "Slack cannot see this channel. A member has to invite Mynaani.";
		case "is_archived":
			return "This channel is archived. Somebody has to unarchive it before Mynaani can join.";
		case "missing_scope":
		...
		case "unknown_bot_user":
			return "Slack did not report which user Mynaani is.";
		default:
...
export async function createSlackChannel(
	...
	if (!token) {
		return {
			error: isPrivate
				? "This workspace did not grant Mynaani permission to create a private channel."
